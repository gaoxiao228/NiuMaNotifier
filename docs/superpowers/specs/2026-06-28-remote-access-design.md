...
Web 外部客户端的页面结构、设备控制台布局、ContinueWork 参考迁移、E2EE RPC 客户端和 i18n 约束详见 [NiuMaNotifier 远程 Web 控制台设计](./2026-06-28-remote-web-client-design.md)。
本机 RemoteAgent 的模块边界、浏览器登录绑定、凭据存储、WSS 状态机、RPC 映射、权限和审计详见 [NiuMaNotifier 本机 RemoteAgent 设计](./2026-06-28-remote-agent-design.md)。
...
```text
用户在 NiumaNotifier 设置页点击登录
  -> NiumaNotifier 创建一次性桌面绑定会话
  -> 打开系统浏览器访问远程服务端登录页
  -> 用户在浏览器完成邮箱密码登录
  -> 服务端自动把当前 NiumaNotifier 设备绑定到该账号
  -> NiumaNotifier 轮询拿到 device_id 和 device_token
  -> 自动注册或绑定当前设备
  -> 自动开启远程访问
  -> 自动开启远程控制
...
本机设置页不内嵌邮箱密码表单，也不保存用户密码。账号登录和注册统一在远程服务端 Web 页面完成。本机只保存当前服务端地址、账号摘要、`device_id` 和 `device_token`。

## 本机组件

第一版 RemoteAgent 是宿主内置模块，不插件化。它不使用 `plugin.json`，不进入插件运行管理器，也不把 `device_token` 放入插件数据目录。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/v1/desktop-login/start` | 本机创建浏览器登录绑定会话。 |
| POST | `/api/v1/desktop-login/complete` | 浏览器登录后完成设备绑定。 |
| POST | `/api/v1/desktop-login/poll` | 本机轮询绑定结果。 |
