...
| 查询会话详情和控制能力 | `GET` | `/api/v1/session_detail` | JSON envelope | 实验 |
| 向托管 Codex 会话续写指令 | `POST` | `/api/v1/session-control/send-instruction` | JSON envelope | 实验 |
| 回答托管 Codex 等待输入 | `POST` | `/api/v1/session-control/answer-input` | JSON envelope | 实验 |
| 中断托管 Codex 当前 turn | `POST` | `/api/v1/session-control/interrupt` | JSON envelope | 实验 |
...
只能回到工具内处理的等待事件会使用 `handling: "tool"` 且 `actionable: false`，例如 watcher fallback 授权或当前等待输入。此时外部系统应展示 `interaction.message`，不要展示同意、拒绝或提交按钮。
...
| `interaction` | object/null | 当前等待授权/等待输入项的交互能力快照。展示操作时以它为准。 |
| `completion_reason` | string/null | 完成原因。 |
| `failure_reason` | string/null | 失败原因。 |

外部系统应直接使用 `status` 判断主状态，不要根据 `event_type` 自行推导。`detail.interaction` 是主状态当前阻塞项的展示快照；需要实时弹窗或处理授权时，仍建议订阅 `/api/v1/events/stream` 并消费事件本身的 `interaction`。
