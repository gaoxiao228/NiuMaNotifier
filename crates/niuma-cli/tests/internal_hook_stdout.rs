...
#[test]
fn internal_claude_hook_is_routed_and_keeps_stdout_empty_for_non_permission_events() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_niuma"))
        .args([
            "internal",
            "hook",
    ...
    child
        ...
        .expect("stdin 必须可写")
        .write_all(
            br#"{"hook_event_name":"Stop","session_id":"claude-s1","cwd":"/tmp/demo","last_assistant_message":"done"}"#,
        )
        .expect("测试 payload 必须可写入");
    ...
    assert!(
        !stderr.contains("Claude Code internal hook 尚未实现"),
        "Claude hook 应进入 runtime，而不是返回未实现：{stderr}"
    );
}
