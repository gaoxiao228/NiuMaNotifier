...
pub(crate) fn codex_send(wrapper_session_id: String, message: String) -> ApiResponse<Value> {
    match codex_send_inner(&wrapper_session_id, &message) {
        Ok(value) => ApiResponse::ok(value),
        Err(error) => ApiResponse::fail(ApiErrorCode::BusinessValidation, error),
    }
}

fn codex_send_inner(wrapper_session_id: &str, message: &str) -> Result<Value, String> {
    ...
    let response = send_instruction_to_control_socket(&session.control_socket, message)?;
...
#[cfg(unix)]
fn send_instruction_to_control_socket(
    control_socket: &str,
    message: &str,
) -> Result<Value, String> {
...
fn send_instruction_to_control_socket(
    ...
    Err("niuma-codex send 当前仅支持 Unix socket 平台".to_string())
}
