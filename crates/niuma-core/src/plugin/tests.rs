...
fn external_manifest_cannot_override_builtin_codex_plugin() {
    ...
    let registry = PluginRegistry::with_builtin_plugins().discover_external_plugins(temp.path());
...
fn management_items_enable_non_tool_plugins_by_default_until_explicitly_disabled() {
    ...
    let items = registry.management_items(&ListenerConfig::default(), &enabled, &HashMap::new());
